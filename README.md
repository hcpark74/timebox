...
- 주간 캘린더에서 날짜 선택
- 주간 캘린더에서 날짜 버튼 및 키보드 선택
- 선택한 날짜 기준 일정 목록 확인
- `오늘의 핵심` / `할 일 목록`으로 태스크 분류
- 새 태스크 추가 및 삭제
- 드래그 앤 드롭 또는 버튼으로 우선순위 이동
- `오늘의 핵심` 수동 재정렬 및 `할 일 목록` 접기/펼치기
- 시간 배치 화면의 주간 이동 버튼, 날짜 바, 고정 블록 선택으로 일정 배정
- 타임라인에서 완료 상태 토글
- 하루 마무리와 다음 날 계획 이어가기
- 프로필 화면에서 동기화 ID 복사/수정, 카테고리별 태스크 수 확인
...
- `오늘 일정`: 하루 타임라인, 완료 체크, 하루 마무리
- `캘린더`: 주간 날짜 선택, 키보드 날짜 이동, 일정 목록
- `할 일 정리`: 오늘의 핵심/할 일 목록 관리, 어제 남은 일 확인, 태스크 추가, 수동 재정렬, 목록 접기
- `시간 배치`: 주간 날짜 이동, 고정 블록 카드 기반 배정/해제 스케줄링
- `내 정보`: 동기화 ID 복사/수정, 카테고리별 태스크 수
